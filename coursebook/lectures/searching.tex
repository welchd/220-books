...
So hash functions are not $1$-to-$1$; that is, two keys may be mapped to the same index -- a \defnfont{collision}. 
...
\begin{itemize}
\item \defnfontX{Open addressing}\index{open addressing} uses no extra space. Every element is stored in the hash table. 
If it gets overfull, we can reallocate space and rehash.
\item \defnfontX{Chaining}\index{chaining} uses an ``overflow" list for each element in the hash table.
...
\subsection{Plots of $Q(m, n)$ against $n$, $m=25,100,400,1600$}
\begin{figure}[htb]
\centering
...
\begin{Boxample}[6]
Java programming language String \texttt{hashcode} function computes the address of a string 
$s=s[0]\cdots s[n-1]$ using integer arithmetic via the formula 
$s[0]*31^{n-1} + s[1]*31^{n-2} + ... + s[n-1]$. Can you find two 2-character strings with the same hashcode? 
