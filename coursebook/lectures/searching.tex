...
\begin{Boxample}[4]
Suppose that we hash dictionary words by letting $h(w)$ be the address of the last letter of $w$, converted to lower case (so there are 26 slots in the hash table, numbered $0,\dots , 25$ to correspond to $a, b, \dots, z$ respectively. 
...
\section{Collisions}
The number of possible keys is usually much large than the actual number of keys so we do not want to allocate an array large enough to fit all possible keys. 
...
\begin{Boxample}[4]
If we use the hash function for dictionary words given above, and hash the following 14 words using chaining, what is the length of the longest chain formed?
The words are:
...
\begin{itemize}
	...
	\begin{itemize}
		\item The most common probing method is \defnfont{linear probing}, which moves left
		 ...
		 address. This is easy to implement but leads to \boldfont{clustering}.
...
For example, $Q(366, 180) \approx 0.4487
 \times 10^{-23}$  (negligible chance of no collision when 180 balls thrown into 366 boxes) while $Q(366, 24) \approx  0.4627$ (slightly more likely than not to have a collision when throwing 24 balls in 366 boxes).
...
We summarise the table implementations that we have looked at so far.

{\color{red} TODO: is this table helpful here since most of the discussion hasn't been about worst case?}
