...
\end{itemize}
%\item BSTs are very versatile. They are good for sorting. An inorder traversal 
%of the tree yields the keys in sorted order. Also, BSTs model the behaviour of 
%quicksort.
%\item Main problem with BSTs: they can become unbalanced by insertions and 
%deletions.
\end{itemize}
...
%\item A BST built by $n$ insertions of random keys has height in 
%$\Theta(\log n)$ (see next slide). So randomly grown trees are not too 
%unbalanced.
%\item However deletions can mess this up, and are hard to analyse.
...
\paragraph{Extra: height of a balanced (AVL) tree}
\begin{itemize}
...
\item We often use the \defnfont{simple uniform hashing} model. That is, each of 
...
of comparable size.
\item Define the \defnfont{load factor} to be $\lambda := n/m$.
\end{itemize}
...
\item If we want to print out all items of a table in sorted order, what is the best data structure?
\item If we don't mind an occasional long wait and mostly do lookups, which is best?
\item If we do a lot of insertions and deletions and few lookups, and are risk-averse, which is best?
\end{itemize}
