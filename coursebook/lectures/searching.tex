...
$$
\frac{1}{n} \sum_{k=1}^n \frac{k}{m} = \frac{n+1}{2m} \approx  \frac{n}{2m}  = \lambda/2 \text{.}
$$
...
Uniform hashing is a theoretical model and cannot be implemented practically but is approximation when double hashing is used.
...
Balanced BST & $\log n$ & $\log n$ & $\log n$ & $n$ \\
Hash table (chaining) & $\lambda$ & $\lambda$ & $\lambda$ & $m$ \\
\hline
\end{tabular}}
\end{table}
