...
We consider only two elementary operations: a \defnfont{comparison} of two items and a \defnfont{move} of an item.
...
\State $k \gets a[i]$ \Comment{save value so we don't overwrite}
\State $j \gets i - 1$
...
On an array, we can reduce the number of comparisons by using binary search to find 
...
It is clearly better than insertion sort.

\chapter{Divide and conquer sorting algorithms}  %----------------------------
Here we look at two ``industrial strength" sorting algorithms, \defnfont{Mergesort} and \defnfont{Quicksort}.
\begin{itemize}
...
Thus the formula holds for all $n\geq 1$, by \defnfont{mathematical induction}.
\end{Example}
...
jo-TODO this example is in the wrong place, doesn't make much sense until we have done the partition algorithm (and that needs its own Boxample too).
\begin{Boxample}[5]
...
\paragraph{What happens if we  use quicksort on a linked list?}
...
\paragraph{How should we choose the pivot?}

If we always choose the first element of the list, it is easy to implement but
...
We can take the median of a sample of (say 3) elements. This reduces the 
likelihood of worst-case behaviour and also makes performance on sorted arrays 
and arrays with equal keys better. 

Choosing an element uniformly at random (or randomly shuffling the list and choosing the first element) 
...
\paragraph{How else might we improve quicksort?}
MCW TODO  timsort, pivot is median
...
What happens when we have many equal keys?
...
jo-TODO position these graphics properly

\begin{Example}
\includegraphics[width=9cm]{figs/selsort-dt}
...
``is $a[i] < a[j]$"?
%\frametitle{Selection sort decision tree for $n=3$}
...
%\frametitle{Insertion sort decision tree for input of size 3}
\includegraphics[width=12cm]{figs/insort-dt}
\end{Example}
...
\paragraph{Worst case running time of comparison-based sorting algorithm}
...
\end{itemize}
% Jo - how about the following Boxample, instead of writing down the idea? Analysis afterwards
% \begin{Boxample}[5]
% Let $Q$ be a priority queue with the methods \texttt{insert} and \texttt{getMin}. 
% Write a sorting algorithm (in pseudo-code) utilising $Q$. 
% \end{Boxample}
...
\paragraph{Array representation of binary heap.}
\begin{itemize}
...
\paragraph{How can we further improve heapsort?}

We can build the heap by repeatedly adding elements to the empty heap.
However, for sorting we only need the heap property at the end. One way is to build a complete binary tree without the heap property, then
...
There is also a way to delete the maximum that is about twice as fast on 
average: it involves swapping down a long way and then swapping back up if 
necessary. See lecturer for more details.
...
The following table gives a summary of sorting algorithms.
\begin{table}[htb] 
...
$r$th key from an input list (the $r$th \defnfont{order statistic}). 
