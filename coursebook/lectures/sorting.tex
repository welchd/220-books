...
We consider only two elementary operations: 
	a \defnfont{comparison} of two items and a \defnfont{move} of an item.
...
\begin{algorithm}[H]
  \caption{Selection sort
    \label{alg:selsort}}
\begin{algorithmic}[1]
\Function{SelectionSort}{list $a[0..n-1]$}
%\State $n \gets \texttt{a.size()}$ 
\For{$i \gets 0$ \text{to} $n-1$} 
	\State $k \gets $ \texttt{findMax}($a[i..n-1]$)
	\If{$k \neq i$} 
...
\begin{itemize}
	...
	until we find a preceding element which it exceeds, and then swap these 
...
\item Thus the formula holds for all $n\geq 1$, by \defnfont{mathematical induction}.
\end{itemize}
...
%It turns out that for nice functions like these, knowing the answer for 
%$n$ restricted to be a power of $2$ is enough (we can ignore floors and ceilings).
