...
\item We write $\ln = \log_e$ and $\lg = \log_2$. Note that $\log_a x = \log_a b \log_b x$ and $a^x = e^{x\ln a}$. If the base of the logarithm does not matter (for example when we do asymptotic analysis) e often just write $\log$.
\end{itemize}
...
An important technique is \defnfont{L'H\^{o}pital's rule}: $$\lim_{x\to \infty} f(x)/g(x) = \lim_{x\to \infty} f'(x)/g'(x)$$ as long as the latter exists. 
