...
\section{Extra: Asymptotics via integral approximation}
\begin{Boxample}
...
\par\vspace*{8cm}\hfill\footnotesize See \url{https://youtu.be/BywPWJlUQro}, requires calculus
\end{Boxample}

\begin{Boxample} 
The same integral approximation method shows that $H_n $ is $\Theta(\log n)$ where
$H_n:=1+ 1/2 + 1/3 + \dots + 1/n$ is the $n$th \boldfont{harmonic number}.
\end{Boxample}
