...
\item The time taken in choosing grey nodes is and
$\Omega(n)$. 
...
\item So the running time of \algfont{traverse} is $O(An + Bm)$
and $\Omega(an + bm)$ if adjacency lists are used, and $O(An + Bn^2)$
and $\Omega(an + bn^2)$ if adjacency matrix format is used.

\item for siimple slection rules (as we'll mainly consider) $a, b, A, B$ are all
constant, and so the running time is $\Omega(an + bm)$
