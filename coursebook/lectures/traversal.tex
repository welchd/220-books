...
Most of the work is done in \cref{alg:visit} (\algfont{visit}) which takes a starting node and explores the portion of the subgraph reachable from that node.
...
A call to \algfont{visit} creates a subdigraph of $G$ that is a tree: the
...
Each white node chosen in \algfont{traverse} is the root of a tree in the call to \algfont{visit}. Eventually, we obtain a
...
\begin{center} 
jo TODO: can you organise these figures a bit better? Is it possible to save space?

\includegraphics{traversalAlgo1} 

\includegraphics{traversalAlgo2}
 
\includegraphics{traversalAlgo3}
 
\includegraphics{traversalAlgo4}
 
\includegraphics{traversalAlgo5}
 
\includegraphics{traversalAlgo6}
  
\includegraphics{traversalAlgo7} 
\end{center}
%\input{figs/generalTraversal} 
...
\begin{Boxample}
jo TODO: could clean this up so figure is on left, arcs to be named are on right
Classify the  arcs according to the definitions in the theorem \\
$(a, e) $ is a tree arc \\  $(d, b)$ is a \\  $(c, a)$ is a  

%\begin{center}
\includegraphics{traversalAlgoResult} 
%\end{center}
\end{Boxample}
...
Here consider that the schemes are simple and constant time. % TODO strange wording
...
It does not matter what choice is made but our \defnfont{convention is to choose the one with
lowest index} (recall nodes have indices $0, \dots, n - 1$). 

These choices can be made in constant time so that the running time is  $\Omega(n + m)$ (assuming adjacency lists) and DFS is linear in the size $+$ order of a digraph. 
...
\begin{Boxample}[2]
jo TODO: omit edges+arcs from graph on right.
Use the nodes on the right to draw the search tree you obtain by running DFS on hte graph on the left, starting at vertex $0$. Use dashed edges to indicate edges that are not arcs in the search tree.
\begin{center}
\includegraphics{DFSalgoGraph} 
\end{center}
Find a tree arc, cross arc, forward arc and back arc (or say if that type of arc does not exist for that traversal).
\end{Boxample}

For a general digraph, and unlike the examples given here, all nodes may not be reachable from the starting node. 
In this case, we get a search forest where the number of trees in the forest is the number of calls made to \algfont{DFSvisit}. %produces a single search tree
%while the algorithm \algfont{DFS} (a special case of \algfont{traverse}) repeatedly chooses a root and runs the associated visit algorithm, producing the search forest.
...
\Function{DFS}{digraph $G$}
	...
	\For{$u \in V(G)$}  
		\State $\colour[u] \gets $ WHITE; $\pred[u] \gets $ NULL \Comment{initialise arrays}
	\EndFor
	\State $\timeVar \gets 0$ \Comment{Time will increment at every niode colour change}
	\For{$s \in V(G)$} 
		\If{$\colour[s] = $ WHITE}  \Comment{find a WHITE node}
			\State \algfont{DFSvisit}$(s)$ \Comment{start traversal from $s$}
		\EndIf
	\EndFor
...
\Function{DFSvisit}{node $s$}
	...
	\State $S$.\algfont{insert}$(s)$ \Comment{put $s$ in stack}
	\While{\textbf{not} $S$.\algfont{isEmpty}$()$}
		\State $u \gets S$.\algfont{peek}$()$  \Comment{get node at front of stack}
		\If{there is a neighbour $v$ with $\colour[v] = $ WHITE}
			...
			\State $S$.\algfont{insert}$(v)$  \Comment{put neighbour in stack}
		\Else
			\State $S$.\algfont{delete}$()$  \Comment{delete node from stack}
...
\Function{recursiveDFSvisit}{node $s$}
	...
	\For{each $v$ adjacent to $s$}
		\If{$\colour[v] = $ WHITE} \Comment{each unvisited neighbour}
			...
			\State \algfont{recursiveDFSvisit}$(v)$ \Comment{now visit neighbour}
		\EndIf
	\EndFor
...
\begin{Boxample}
DFS with \algfont{recursiveDFSvisit}.
\begin{center}
\includegraphics{DFSalgoRecursive}
\end{center}
\end{Boxample}
...
Suppose that we have performed \algfont{BFS} on a graph $G$. 
...
Pseudocode demonstrating the PFS is presented in \cref{alg:PFScode,alg:PFScodeVisit}. 
The subroutine \algfont{setKey} there is the rule for giving the key value when a node is inserted. 
We do not include any code for \algfont{setKey}.
...
then $u$ appears before $v$ in the ordering. It is also known as  a defnfont{topological order} or \defnfont{linear order}.
\end{Definition}
...
\end{Boxample}
% (otherwise we would obtain a topological order and hence the
...
% A digraph and its three strongly connected components. \\
...
We can determine if $G$ is strongly connected or not (i.e., has a single strongly connected component) by running \algfont{BFSvisit} or \algfont{DFSvisit}
...
shortest possible.
\end{Exercise}
% jo asks: do you want this exercise in the cycle lecture?
