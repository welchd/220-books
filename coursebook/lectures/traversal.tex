...
searching as far away from the root as possible. We continue away from the root until we cannot visit
any new nodes, then backtrack as little as possible. The root is the last node to turn black.
...
Both of the choices for grey and white nodes are simple and be made in constant time so that the running time is  $\Omega(n + m)$ (assuming adjacency lists). So BFS and DFS are linear in the size + order of a digraph. 
...
\algfont{BFSvisit}/\algfont{DFSvisit} (special cases of \algfont{visit}) produces a single search tree, while the algorithm \algfont{DFS}/\algfont{BFS} (special cases of \algfont{traverse}) repeatedly chooses a root and runs the associated visit algorithm, producing the search forest.
...
This ``last in, first
out" order is mimicked  by a \boldfont{stack} data structure. We thus store grey nodes in a stack as they are discovered. 

The pseudocode for DFS and DFS visit is in \cref{alg:DFScode}. 
