...
\begin{itemize}
	...
	\itm To mimic BFS, set the key for the node $v$ to be the time that $v$ turns grey. 
...
Consider the DFS tree originating from vertex $0$ of the graph below. % third grpah of \cref{fig:cycle}
Which is the smallest cycle it finds with the back edges? Which smaller cycle does it miss?

jo TODO
% finds only $6$-cycles with the back edges (even
...
jo TODO-add in graph here to illustrate
\end{Boxample}
...
A digraph and its three strongly connected components. Note that there are
