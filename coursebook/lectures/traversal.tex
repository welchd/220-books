...
Since $H$ is a DAG, we can find a topological order of it using the reverse order of finishing times of a DFS. WE also note that the the strong components of $G$ are the same as the strong components of its reverse, $G_r$. So, as before, starting a DFS in $C_m$ in $G_r$ means we would only visit nodes in $C_m$ and so on. This leads to the following solution, due to Tarjan.
