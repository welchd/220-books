...
Traversals involve visiting each node of a digraph in a 
systematic way using only the structure of the digraph. This is a very common task when dealing with graphs and 

The important requirements are that we must be
...
There are several ways to perform a traversal.  We start with a genral framework for traversals then present the
...
in Figure~\ref{fig:travcols}.

\begin{figure}
%\centerline{\includegraphics[width=4in]{figs/wNodeStates.eps}}
%\centerline{\includegraphics[width=3.6in]{figs/wNodeStatesBW.eps}}
\centerline{\includegraphics[width=3.6in]{figs/wNodeStatesBW2.eps}}
...
The above is formalized in Figure~\ref{fig:travcode}. Here
...
in later sections. Figure~\ref{fig:travdecomp} illustrates the first part.
...
and digraph $G_2$  are displayed in Figure~\ref{fig:graphEx2-BFS}. The
...
Figure~\ref{fig:DFScode}. Note how the algorithm for \algfont{visit} has
...
\> \> \> $colour[v] \gets $ GREY; $pred[v] \gets u$ \\ 
\> \> \> $seen[v] \gets time$; $time \gets time + 1$ \\
\> \> \> S.\algfont{insert}($v$) \\
\> \> \textbf{else} \\
\> \> \> S.\algfont{delete}() \\
\> \> \> $colour[u] \gets $ BLACK \\
...
$G_1$ of Figure~\ref{fig:graphExample2}, we label vertex 0 with 1,
...
\set{7,8,9}. These are indicated in Figure~\ref{fig:graphEx2-BFS}.
...
Figure~\ref{fig:PFScode}. The subroutine \algfont{setKey} there is the
...
%\subsection*{Exercises}
\chapter{Lecture: Cycles, Girth and Topological Sort}
...
expression like that shown in Figure~\ref{fig:prec}.
...
components are displayed in Figure~\ref{fig:scc}. Note that there are
...
is acyclic. See Figure~\ref{fig:sccdecomp}. Note that the converse
...
in Figure~\ref{fig:travdecomp}. In that case, if we shrink each
...
\begin{Example}
\label{eg:cycles}
In Figure~\ref{fig:cycle} are three (di)graphs.  The first has no cycles 
(it is a free tree), the second is a DAG of girth 3, and the third has girth 4.
\end{Example}

\begin{figure}

\centerline{\IpeScale{80}\Ipe{./figs/graphEx3.ipe}}
...
Figure~\ref{fig:cycle} finds only $6$-cycles with the back edges (even
...
\begin{Example}
The graph in Figure~\ref{fig:bipartite} is bipartite.  The isolated
...
\begin{Example}
The graph in Figure~\ref{fig:bipartite} has a 2-colouring as indicated.
\end{Example}
...
In Figure~\ref{fig:matchbipartite} we illustrate the difference between
a maximal and maximum matching in the setting of Example~\ref{ex:mp}.
...
Figure~\ref{fig:matchbipartite}, a maximal matching may have fewer
...
Figure~\ref{fig:matchbipartite}.  It contains three edges but only the
...
path exists starting at $v$.  This algorithm is given in Figure~\ref{fig:augpath}.
...
Figure~\ref{fig:augpath}, does find an augmenting path if one exists.
...
illustrated in Figure~\ref{fig:augtree}.  The status of the nodes are ODD 
