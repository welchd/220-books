...
\begin{figure}[h]
\centering
\includegraphics[width = \textwidth]{decomposition2}
\caption{decomposing a graph into its strongly connected component. If each $C_i$ is considered a single node, this decomposiion is a DAG.}
 \label{fig:sccdecomp}
\end{figure}
...
\item List the nodes of $G$ in the reverse order of finishing (those that finish first listed last).
\item Find $G_r$, the reverse of $G$. 
\item Run DFS on $G_r$ always choosing a root (starting) node from the unvisited node that occurs first on the list. 
\item This produces a forest $F_r$ and each tree in the forest is a strongly connected component of $G$.
\end{itemize}
...
\begin{Boxample}[5]
Run the algorithm above on the graph shown (starting in the initial DFS at node $a$) to find all strongly connected components. 

jo TODO: just clip this figure (or redraw) showing only $G$.

\includegraphics[width = 10cm]{sccboxample}
\end{Boxample}
