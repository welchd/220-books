...
\begin{itemize} 
  \item \defnfontX{White nodes}\index{white node} have not yet been visited;
  \item \defnfontX{Grey nodes}\index{grey node} (or \boldfont{frontier nodes}) have been visited but may have
  out-neighbours that are white;
  \item \defnfontX{Black nodes}\index{black node} have been visited and all their out-neighbours have been visited too (so are not white). 
\end{itemize} 
...
Suppose that we have performed \texttt{DFS} on a digraph $G$.
 Let $(v, w)\in E(G)$. Show that the following statements are true.
\begin{itemize}
...
\begin{Theorem} \label{thm:BFS-cycle} 
